...
def minEntropy(wordDic,docList):
    for wid,v in wordDic.items():
        ...
        for i in range(len(classList)):
            for doc in docList:
                if doc[2]==classList[i] and wid in doc[0]:
                    numDocEachC[i]+=1
        #print '1', numDocEachC
        allDoc=float(sum(numDocEachC))#fenmu, for all doc ,how many is there contain the wid
        numDocEachC=[i/(allDoc+0.0001) for i in numDocEachC];#print 'numDock',numDock
        #print '2',numDocEachC
...
def calcProb(stumpInfo,docList):
    ...
    stumpInfo[3]=0.5*math.log(eps+p1L/(1-p1L+eps))
    stumpInfo[4]=0.5*math.log(eps+p1R/(1-p1R+eps))
    #print stumpInfo
